(() => {
  ...
  const translations = {
    uz: {
      ...
      anime:{eyebrow:'TOMOSHA RO‘YXATI / 04',title:'Sevimli Anime va Donghua',code:'04 TA NOM',animeLabel:'01 / ANIME',animeTitle:'Anime',donghuaLabel:'02 / DONGHUA',donghuaTitle:'Donghua',titles2:'02 TA NOM'},
    ...
    en: {
      ...
      anime:{eyebrow:'WATCHLIST / 04',title:'Favorite Anime & Donghua',code:'04 TITLES',animeLabel:'01 / ANIME',animeTitle:'Anime',donghuaLabel:'02 / DONGHUA',donghuaTitle:'Donghua',titles2:'02 TITLES'},
    ...
    ru: {
      ...
      anime:{eyebrow:'СПИСОК ПРОСМОТРА / 04',title:'Любимое аниме и дунхуа',code:'04 НАЗВАНИЯ',animeLabel:'01 / АНИМЕ',animeTitle:'Аниме',donghuaLabel:'02 / ДУНХУА',donghuaTitle:'Дунхуа',titles2:'02 НАЗВАНИЯ'},
    ...
    ja: {
      ...
      anime:{eyebrow:'ウォッチリスト / 04',title:'好きなアニメ＆ドンファ',code:'04 タイトル',animeLabel:'01 / アニメ',animeTitle:'アニメ',donghuaLabel:'02 / ドンファ',donghuaTitle:'ドンファ',titles2:'02 タイトル'},
    ...
    zh: {
      ...
      anime:{eyebrow:'观看列表 / 04',title:'喜欢的动画与动漫',code:'04 部作品',animeLabel:'01 / 动画',animeTitle:'动画',donghuaLabel:'02 / 国创动画',donghuaTitle:'国创动画',titles2:'02 部作品'},
  ...
  const normalizeBrowserLanguage = value => {
    const code = String(value || '').toLowerCase().split('-')[0].split('_')[0];
    return SUPPORTED.includes(code) ? code : null;
  };
  const browserLanguages = () => (navigator.languages || [navigator.language || '']).map(normalizeBrowserLanguage).filter(Boolean);
  const detectByBrowser = () => browserLanguages()[0] || null;
  async function detectByIP() {
    try {
      const controller = new AbortController();
      const timer = setTimeout(() => controller.abort(), 1800);
      const response = await fetch('https://ipapi.co/json/', {headers:{Accept:'application/json'},signal:controller.signal,cache:'no-store'});
      clearTimeout(timer);
      if (!response.ok) return null;
      const data = await response.json();
      return COUNTRY_LANGUAGE[String(data.country_code || '').toUpperCase()] || null;
    } catch (_) { return null; }
  }
  function applyLanguage(lang, save = true) {
    ...
    $$('[data-i18n-aria-label]').forEach(el => { const value=get(translations[lang],el.dataset.i18nAriaLabel); if(value!=null) el.setAttribute('aria-label',value); });
    $$('[data-i18n-alt]').forEach(el => { const value=get(translations[lang],el.dataset.i18nAlt); if(value!=null) el.setAttribute('alt',value); });
  ...
  languageButton?.addEventListener('click', e => { e.stopPropagation(); const open=languageMenu.classList.toggle('open'); languageButton.setAttribute('aria-expanded',String(open)); });
  languageMenu?.querySelectorAll('[data-lang]').forEach(btn => btn.addEventListener('click', () => applyLanguage(btn.dataset.lang, true)));
  ...
  if (savedLang && translations[savedLang]) {
    ...
    // Browser locale is the strongest signal; IP country is used when the browser locale is unavailable.
    const browserLang = detectByBrowser();
    applyLanguage(browserLang || 'en', false);
    if (!browserLang) detectByIP().then(ipLang => { if (!localStorage.getItem('site-language')) applyLanguage(ipLang || 'en', false); });
  }
